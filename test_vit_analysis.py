...
# ── caching wrapper ───────────────────────────────────────────
from scenarios.utils.cached_loader import cache_dataset
...
def train_task(model, tr_loader, device, epochs=40, lr=3e-4):
    ...
    for ep in range(epochs):
        ...
        for x, y in tr_loader:
            ...
            corr += model(x).argmax(1).eq(y).sum().item(); tot += y.size(0)
        sched.step()
        if ep in {0, 9, 19, 29, 39}:
            print(f"    epoch {ep+1:2d}: train {100*corr/tot:5.1f}%")
